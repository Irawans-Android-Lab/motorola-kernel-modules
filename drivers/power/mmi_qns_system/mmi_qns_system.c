...
#define CHARGE_CURRENT_PROP	POWER_SUPPLY_PROP_CHARGE_CONTROL_LIMIT_MAX
...
#ifdef CONFIG_ENABLE_QNOVO_CONTROL_CHARGE
static int qns_set_ibat(int ibatmA)
{
			...
			CHARGE_CURRENT_PROP, &propVal) != 0)
	{
		pr_info("QNS: ERROR: unable to set charging current! Does " IBATMANAME " have "
				"POWER_SUPPLY_PROP_CHARGE_CONTROL_LIMIT_MAX property?");
		return QNS_ERROR;
	}
	return QNS_OK;
}
...
static int qns_set_vbat(int vbatmV)
{
			...
			CHARGE_VOLTAGE_PROP, &propVal) != 0)
	{
		pr_info("QNS: ERROR: unable to set charging voltage! Does " IBATMANAME " have "
				"POWER_SUPPLY_PROP_VOLTAGE_MAX property?");
		return QNS_ERROR;
	}
	return QNS_OK;
}
...
static int qns_get_battery_type(const char **battery_type)
{
	...
	{
		if(power_supply_get_property(data.battery_psy,
				POWER_SUPPLY_PROP_MODEL_NAME, &ret) != 0)
		{
			pr_info("QNS: ERRROR: unable to read battery POWER_SUPPLY_PROP_MODEL_NAME property.");
			*battery_type = "Unknown";
			retVal = QNS_ERROR;
		}
		else
			      ...
			      const char *buf, size_t count)
{
	...
	{
		data.charge_wakelock = wakeup_source_register(NULL, "QnovoQNS-CHG");
