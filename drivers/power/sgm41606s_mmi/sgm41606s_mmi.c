...
static int sgm41606S_enable_charge(struct sgm41606S_chip *sgm, bool en)
{
    int ret;
    dev_info(sgm->dev,"%s:%d",__func__,en);

    ret = sgm41606S_field_write(sgm, B_CHG_EN, !!en);

    return ret;
}

static int sgm41606S_check_charge_enabled(struct sgm41606S_chip *sgm, bool *enabled)
{
    int ret, val;
...
static void sgm41606S_check_fault_status(struct sgm41606S_chip *sgm)
{
    ...
    dev_err(sgm->dev,"sgm41606S_check_fault_status  %d\n", ret);
}
...
static int sgm41606S_parse_dt(struct sgm41606S_chip *sgm, struct device *dev)
{
    ...
    struct {
        ...
        {"sgm,sgm41606S,vac1-ovp", &(sgm->cfg.vac1_ovp)},
        ...
        {"sgm,sgm41606S,ibat-ucp-dis", &(sgm->cfg.ibat_ucp_alm_dis)},
        {"sgm,sgm41606S,ibat-ucp-alm", &(sgm->cfg.ibat_ucp_alm)},
