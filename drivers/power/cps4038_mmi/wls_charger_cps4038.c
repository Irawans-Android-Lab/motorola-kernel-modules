...
#endif
#endif
static irqreturn_t wls_hal_irq1_handler(int irq, void *dev_id)
{
	struct cps_wls_chrg_chip *chip = dev_id;
	int hal_detected = gpio_get_value(chip->wls_hal_int1);

	cps_wls_log(CPS_LOG_DEBG, "Detected an hal 1 event:%d\n",hal_detected);

	return IRQ_HANDLED;
}

static irqreturn_t wls_hal_irq2_handler(int irq, void *dev_id)
{
	struct cps_wls_chrg_chip *chip = dev_id;
	int hal_detected = gpio_get_value(chip->wls_hal_int2);

	cps_wls_log(CPS_LOG_DEBG, "Detected an hal 2 event:%d\n", hal_detected);
