...
#define GDX_VTG_MIN_UV                      3300000
#define GDX_VTG_MAX_UV                      3300000
...
static int goodix_ts_power_init(struct goodix_ts_core *core_data)
{
	...
	if (strlen(ts_bdata->avdd_name)) {
		...
		if (regulator_count_voltages(core_data->avdd) > 0) {
				ret = regulator_set_voltage(core_data->avdd, GDX_VTG_MIN_UV,
																		GDX_VTG_MAX_UV);
				if (ret) {
						ts_err("avdd regulator set_vtg failed ret=%d", ret);
