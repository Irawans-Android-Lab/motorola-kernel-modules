...
*****************************************************************************/
#define FT_TP_DATA_DUMP "fts_data"
...
static struct ft_tp_data_dump *ft_tp_data_dump_p = NULL;
...
/* add tp_data_dump Start*/

static int ft_tp_get_diffdata(int *data, int byte_num)
{
	int ret = 0;
	u8 old_mode = 0;
	struct fts_test *tdata = fts_ftest;
...
static int ft_tp_get_rawdata(int *data, int byte_num) {
	int ret = 0;
	struct fts_test *tdata = fts_ftest;
	FTS_TEST_FUNC_ENTER();
	ret = start_scan();
...
{
	int row = 0, col = 0;
	u8 tx_num = ft_tp_data_dump_p->tx_num;
	u8 rx_num = ft_tp_data_dump_p->rx_num;
	int *rawdata = ft_tp_data_dump_p->rawdata;
	int *diffdata = ft_tp_data_dump_p->diffdata;
	...
	if (file->count < file->size) {
		if (!IS_ERR_OR_NULL(ft_tp_data_dump_p->rawdata))
			kfree(ft_tp_data_dump_p->rawdata);
		if (!IS_ERR_OR_NULL(ft_tp_data_dump_p->diffdata))
			kfree(ft_tp_data_dump_p->diffdata);
		ft_tp_data_dump_p->rawdata = NULL;
		ft_tp_data_dump_p->diffdata = NULL;
	}
	return 0;
}

int tp_data_dump_capture ()
{
	int ret = 0;
	u8 tx_num = 0;
	u8 rx_num = 0;
	...
	ft_tp_data_dump_p->tx_num = tx_num;
	ft_tp_data_dump_p->rx_num = rx_num;

	//malloc buffer
	ft_tp_data_dump_p->rawdata = (int *)kzalloc(tx_num * rx_num * sizeof(int), GFP_KERNEL);
	if (IS_ERR_OR_NULL(ft_tp_data_dump_p->rawdata)) {
	...
	ft_tp_data_dump_p->diffdata = (int *)kzalloc(tx_num * rx_num * sizeof(int), GFP_KERNEL);
	if (IS_ERR_OR_NULL(ft_tp_data_dump_p->diffdata)) {
	...
	//get rawdataot
	ret = ft_tp_get_rawdata(ft_tp_data_dump_p->rawdata, tx_num * rx_num * 2);
	if (ret < 0) {
		...
		for (col = 0; col < tx_num; col++) {
			for (row = 0; row < rx_num; row++) {
				FTS_INFO("Rawdata:%6d", ft_tp_data_dump_p->rawdata[col*row+row]);
			}
			FTS_INFO("Raw_col:%d",col);
		}
		//if (!IS_ERR_OR_NULL(ft_tp_data_dump_p->rawdata))
		kfree(ft_tp_data_dump_p->rawdata);
		ft_tp_data_dump_p->rawdata = NULL;
	...
	//get diffdata
	ret = ft_tp_get_diffdata(ft_tp_data_dump_p->diffdata, tx_num * rx_num * 2);
	if (ret < 0) {
		FTS_TEST_ERROR("lct_tp_get_rawdata() failed, ret = %d", ret);
		goto err_get_data_fail;
	}
		...
		for (col = 0; col < tx_num; col++) {
			for (row = 0; row < rx_num; row++) {
				FTS_INFO("Diffdata:%6d", ft_tp_data_dump_p->diffdata[col*row+row]);
			}
			FTS_INFO("Diff_col:%d",col);
		}
		//if (!IS_ERR_OR_NULL(ft_tp_data_dump_p->diffdata))
		kfree(ft_tp_data_dump_p->diffdata);
		ft_tp_data_dump_p->diffdata = NULL;
...
static int fts_tp_data_dump_proc_init(void)
{
	...
	FTS_TEST_FUNC_ENTER();
	ft_tp_data_dump_p = (struct ft_tp_data_dump *)kzalloc(sizeof(struct ft_tp_data_dump), GFP_KERNEL);
	if (IS_ERR_OR_NULL(ft_tp_data_dump_p)) {
		FTS_TEST_ERROR("malloc lct_tp_data_dump memory fail");
		ft_tp_data_dump_p = NULL;
		ret = -ENOMEM;
		goto err_malloc_fail;

	}
	ft_tp_data_dump_p->tp_data_dump_proc = proc_create(FT_TP_DATA_DUMP, 0777, NULL, &tp_data_dump_proc_fops);
	if (IS_ERR_OR_NULL(ft_tp_data_dump_p->tp_data_dump_proc)) {
		FTS_TEST_ERROR( "ERROR: create /proc/%s failed.", FT_TP_DATA_DUMP );
		ft_tp_data_dump_p->tp_data_dump_proc = NULL;
		ret = -1;
		goto err_create_procfs_fail;

	}
	FTS_INFO("create /proc/%s", FT_TP_DATA_DUMP);
	return 0;

err_create_procfs_fail:
	if (ft_tp_data_dump_p)
		kfree(ft_tp_data_dump_p);
err_malloc_fail:
	FTS_TEST_FUNC_EXIT();
	return ret;
...
static void fts_tp_data_dump_proc_exit(void)
{
	if (IS_ERR_OR_NULL(ft_tp_data_dump_p))
		return;
	if (!IS_ERR_OR_NULL(ft_tp_data_dump_p->tp_data_dump_proc)) {
		remove_proc_entry(FT_TP_DATA_DUMP, NULL);
		FTS_INFO("remove /proc/%s", FT_TP_DATA_DUMP);
