...
#define GF_LINUX_VERSION "V1.02.04"
...
#define GOODIX_DTS_NAME  "mediatek,fingerprint-goodix"
#ifdef CONFIG_OF
...
static int gf_get_gpio_dts_info(struct gf_device *gf_dev)
{
	...
	node = of_find_compatible_node(NULL, NULL, GOODIX_DTS_NAME);
	if (node) {
	...
	if (gf_dev->rgltr_ctrl_support) {
		...
		if (IS_ERR_OR_NULL(gf_dev->pwr_supply)) {
			...
			ret = of_property_read_u32_array(node, "fp,vdd-current", &current_supply, 1);
			if (ret < 0) {
				pr_warn("  %s : fail to fp,vdd-current \n", __func__);
			}else{
				gf_dev->regulator_current = current_supply;
				ret = regulator_set_load(gf_dev->pwr_supply, gf_dev->regulator_current);
				pr_warn(" %s : vdd regulator_set_load current %d ret=%d \n", __func__, current_supply,ret);
			}
		}
	}
...
MODULE_DESCRIPTION("Goodix Fingerprint chip GF9578 driver");
MODULE_LICENSE("GPL");
