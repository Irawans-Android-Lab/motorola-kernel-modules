...
static void ff_ctl_device_event(struct work_struct *ws)
{
    ...
    PM_WAKEUP_EVENT(g_context->wake_lock, 5000);
...
static int ff_probe(struct spi_device *dev)
{
    ...
    /* Init the wake lock. */
    PM_WAKEUP_REGISTER(&dev->dev, ff_ctl_context.wake_lock, "ff_wake_lock");
    if (!ff_ctl_context.wake_lock) {
        FF_LOGE("wakeup source request failed\n");
        return -ENOMEM;
    }
...
static int ff_remove(struct spi_device *dev)
{
    ...
    PM_WAKEUP_UNREGISTER(g_context->wake_lock);
...
static int __init ff_ctl_driver_init(void)
{
    ...
    if (err < 0) {
        FF_LOGE("spi_register_driver(..) = %d.", err);
    }
