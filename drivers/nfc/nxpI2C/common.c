...
int nfcc_hw_check(struct nfc_dev *nfc_dev)
{
	...
	if (ret <= 0) {
		...
		ret = nfc_dev->nfc_read(nfc_dev, nci_get_version_rsp,
					NCI_GET_VERSION_RSP_LEN,NCI_CMD_RSP_TIMEOUT_MS);
		if (ret <= 0) {
	...
	ret = nfc_dev->nfc_read(nfc_dev, nci_reset_rsp, NCI_RESET_RSP_LEN,NCI_CMD_RSP_TIMEOUT_MS);
	if (ret <= 0) {
	...
	ret = nfc_dev->nfc_read(nfc_dev, nci_reset_ntf, NCI_RESET_NTF_LEN,NCI_CMD_RSP_TIMEOUT_MS);
	if (ret <= 0) {
	...
	pr_err("%s: - NFC reset rsp : NfcNciRx %x %x %x\n",
