...
static void qm35_coredump_check(struct work_struct *work)
{
	...
	spin_unlock(&cd->dump_lock);
	if (cdd->remain) {
		dev_warn(dev, "Coredump incomplete: expected size: %ld, received: %ld, remaining: %lu\n", cdd->size, cdd->offset, cdd->remain);
				       ...
				       struct sk_buff *skb)
{
	...
	if (!list_empty(&cd->dump_list)) {
		...
		if (prev->remain) {
			...
			dev_warn(dev, "Deletes incomplete coredump with len %lu. Missing %lu bytes\n", cdd->size, cdd->remain);
...
static int qm35_coredump_online(struct qm35 *qm35)
{
	...
	if (rc) {
		dev_warn(dev, "Failed to create coredump file (err %d)\n", rc);
	}
	return 0;
}
...
static void qm35_coredump_deregister(struct qm35 *qm35)
{
	...
	list_for_each_entry_safe (cdd, n, &coredump->dump_list, list) {
		dev_warn(dev, "Coredump unread data remain (%lu bytes, %lu missing).\n", cdd->offset, cdd->remain);
		if (!cdd->remain)
