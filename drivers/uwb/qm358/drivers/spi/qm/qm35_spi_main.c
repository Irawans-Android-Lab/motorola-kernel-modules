...
int qm35_set_spi_active(struct qm35_spi *qmspi, int en)
{
	int ret;
	if (en) {
		ret = pinctrl_select_state(qmspi->pinctrl, qmspi->spi_active);
	} else {
		ret = pinctrl_select_state(qmspi->pinctrl, qmspi->spi_default);
	}
	if (ret)
		dev_err(qmspi->base.dev, "Failed to set spi pinctrl %d\n", en);
	return ret;
}
...
static int qm35_spi_driver_probe(struct spi_device *spi)
{
	...
	//Added by motorola begin
	qmspi->pinctrl = devm_pinctrl_get(&spi->dev);
	if (IS_ERR(qmspi->pinctrl)) {
		dev_info(qm->dev, "setup spi pinctrl failed");
		goto err_pinctrl_failed;
	}
	qmspi->spi_active = pinctrl_lookup_state(qmspi->pinctrl, "spi_active");
	if (IS_ERR(qmspi->spi_active)) {
		dev_info(qm->dev, "setup spi pinctrl active failed");
		goto err_pinctrl_failed;
	}
	qmspi->spi_default = pinctrl_lookup_state(qmspi->pinctrl, "spi_default");
	if (IS_ERR(qmspi->spi_default)) {
		dev_info(qm->dev, "setup spi pinctrl default failed");
		goto err_pinctrl_failed;
	}
	qm35_set_spi_active(qmspi, 1);
	//Added by motorola end
...
static int qm35_spi_driver_remove(struct spi_device *spi)
{
	...
	spi_setup(spi);
	//Added by motorola begin
	if (qmspi->spi_default)
		qm35_set_spi_active(qmspi, 0);
	//Added by motorola end
