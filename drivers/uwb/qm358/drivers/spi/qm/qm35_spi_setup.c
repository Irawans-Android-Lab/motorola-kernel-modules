...
int qm35_setup_gpios(struct qm35_spi *qmspi)
{
	...
	if (!gpio || IS_ERR(gpio)) {
		...
		if (gpiod_cansleep(qmspi->ready_gpio))
			dev_warn(dev, "READY GPIO can sleep! This affects performance!\n");
	}
	...
	if (!gpio || IS_ERR(gpio)) {
		...
		if (gpiod_cansleep(qmspi->wakeup_gpio))
			dev_warn(dev, "WAKEUP GPIO can sleep! This affects performance!\n");
	}
	...
	if (!gpio || IS_ERR(gpio)) {
		...
		if (gpiod_cansleep(qmspi->exton_gpio))
			dev_warn(dev, "EXTON GPIO can sleep! This affects performance!\n");
	}
	return 0;
}
