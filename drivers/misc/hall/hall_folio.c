...
static struct hall_sensor_str {
	...
	struct sensors_classdev sensors_folio_cdev;
}* hall_sensor_dev;

#ifdef CONFIG_OF
static const struct of_device_id hall_folio_match[] = {
	{ .compatible = "hall,hall_folio_detect", },
	{}
};
#else
#define hall_folio_match NULL
#endif
MODULE_DEVICE_TABLE(of, hall_folio_match);
...
static struct platform_driver hall_pen_driver = {
	...
	.driver		= {
		.name	= DRIVER_NAME,
		.of_match_table    = hall_folio_match,
	},
};
...
void check_and_send(void)
{
	...
	if(hall_sensor_dev->folio_detect &&
...
void hall_enable(bool enable)
{
	...
	{
		LOG_INFO("hall_folio_sensor enable");
		for (i = 0; i < hall_sensor_dev->gpio_num; i++)
		{
	...
	{
		LOG_INFO("hall_folio_sensor disable");
		for (i = 0; i < hall_sensor_dev->gpio_num; i++)
		{
...
static int hall_folio_enable(struct sensors_classdev *sensors_cdev,
		unsigned int enable)
{
	hall_sensor_dev->report_val = -1;
	hall_sensor_dev->folio_detect = enable;
	hall_enable(enable);
	if (enable == 0)
	{
...
static int hall_sensor_probe(struct platform_device *pdev)
{
	...
	hall_sensor_dev->enable = 0;
	hall_sensor_dev->folio_detect = 0;
	//tcmd node
	...
	hall_sensor_dev->sensors_folio_cdev.sensors_enable = hall_folio_enable;
	hall_sensor_dev->sensors_folio_cdev.sensors_poll_delay = NULL;
	hall_sensor_dev->sensors_folio_cdev.name = hall_sensor_dev->hall_dev->name;
	hall_sensor_dev->sensors_folio_cdev.vendor = "motorola";
	hall_sensor_dev->sensors_folio_cdev.version = 1;
	hall_sensor_dev->sensors_folio_cdev.type = SENSOR_TYPE_MOTO_HALL;
	hall_sensor_dev->sensors_folio_cdev.max_range = "5";
	hall_sensor_dev->sensors_folio_cdev.resolution = "5.0";
	hall_sensor_dev->sensors_folio_cdev.sensor_power = "3";
	hall_sensor_dev->sensors_folio_cdev.min_delay = 0;
	hall_sensor_dev->sensors_folio_cdev.fifo_reserved_event_count = 0;
	hall_sensor_dev->sensors_folio_cdev.fifo_max_event_count = 0;
	hall_sensor_dev->sensors_folio_cdev.delay_msec = 100;
	hall_sensor_dev->sensors_folio_cdev.enabled = 0;

	err = sensors_classdev_register(&hall_sensor_dev->hall_dev->dev, &hall_sensor_dev->sensors_folio_cdev);
	if (err < 0)
