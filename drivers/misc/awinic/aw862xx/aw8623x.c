...
static void aw8623x_reg_unlock(struct aw_haptic *aw_haptic, bool flag)
{
	uint8_t reg_val = 0;
	...
	if (flag) {
		...
		haptic_nv_i2c_writes(aw_haptic, AW8623X_REG_TRIMCFG5, &reg_val, AW_I2C_BYTE_ONE);
	} else {
		/* Lock register */
...
static void aw8623x_interrupt_setup(struct aw_haptic *aw_haptic)
{
	...
	haptic_nv_i2c_reads(aw_haptic, AW8623X_REG_SYSINT, &reg_val, AW_I2C_BYTE_ONE);
	aw_info("reg SYSINT=0x%02X", reg_val);
	/* edge int mode */
	haptic_nv_i2c_write_bits(aw_haptic, AW8623X_REG_SYSCTRL3, AW8623X_BIT_SYSCTRL3_INTMODE_MASK,
				 AW8623X_BIT_SYSCTRL3_INTMODE_POS_EDGE);
	/* int enable */
	haptic_nv_i2c_write_bits(aw_haptic, AW8623X_REG_SYSINTM,
				 (AW8623X_BIT_SYSINTM_UVLM_MASK & AW8623X_BIT_SYSINTM_FF_AEM_MASK &
				  AW8623X_BIT_SYSINTM_FF_AFM_MASK & AW8623X_BIT_SYSINTM_OCDM_MASK &
				  AW8623X_BIT_SYSINTM_OTM_MASK & AW8623X_BIT_SYSINTM_DONEM_MASK),
				 (AW8623X_BIT_SYSINTM_UVLM_ON | AW8623X_BIT_SYSINTM_FF_AEM_OFF |
				  AW8623X_BIT_SYSINTM_FF_AFM_OFF | AW8623X_BIT_SYSINTM_OCDM_ON |
				  AW8623X_BIT_SYSINTM_OTM_ON | AW8623X_BIT_SYSINTM_DONEM_OFF));
}

static void aw8623x_set_rtp_data(struct aw_haptic *aw_haptic, uint8_t *data, uint32_t len)
{
...
static uint8_t aw8623x_get_glb_state(struct aw_haptic *aw_haptic)
{
	...
	haptic_nv_i2c_reads(aw_haptic, AW8623X_REG_GLBRD5, &state, AW_I2C_BYTE_ONE);
...
static uint8_t aw8623x_rtp_get_fifo_afs(struct aw_haptic *aw_haptic)
{
	...
	haptic_nv_i2c_reads(aw_haptic, AW8623X_REG_SYSST, &reg_val, AW_I2C_BYTE_ONE);
...
static uint8_t aw8623x_rtp_get_fifo_aes(struct aw_haptic *aw_haptic)
{
	uint8_t reg_val = 0;

	haptic_nv_i2c_reads(aw_haptic, AW8623X_REG_SYSST, &reg_val, AW_I2C_BYTE_ONE);
	reg_val &= AW8623X_BIT_SYSST_FF_AES;
	reg_val = reg_val >> 4;
...
static void aw8623x_play_stop(struct aw_haptic *aw_haptic)
{
	...
	haptic_nv_i2c_writes(aw_haptic, AW8623X_REG_PLAYCFG4, &val, AW_I2C_BYTE_ONE);
	aw8623x_raminit(aw_haptic, false);
	while (cnt) {
		haptic_nv_i2c_reads(aw_haptic, AW8623X_REG_GLBRD5, &val, AW_I2C_BYTE_ONE);
...
static void aw8623x_vbat_ref_config(struct aw_haptic *aw_haptic, uint16_t vbat_ref)
{
	uint8_t reg_val = 0;
	...
	haptic_nv_i2c_write_bits(aw_haptic, AW8623X_REG_DETCFG1,
					AW8623X_BIT_DETCFG1_VBAT_REF_MASK, reg_val);
}
...
static void aw8623x_play_mode(struct aw_haptic *aw_haptic, uint8_t play_mode)
{
	...
	case AW_RAM_MODE:
		...
		aw8623x_auto_brk_config(aw_haptic, false);
		haptic_nv_i2c_write_bits(aw_haptic, AW8623X_REG_PLAYCFG3,
					 AW8623X_BIT_PLAYCFG3_PLAY_MODE_MASK,
					 AW8623X_BIT_PLAYCFG3_PLAY_MODE_RAM);
		aw8623x_vbat_mode_config(aw_haptic, AW_CONT_VBAT_SW_COMP_MODE);
		break;
	case AW_RAM_LOOP_MODE:
		aw_info("enter ram loop mode");
		aw_haptic->play_mode = AW_RAM_LOOP_MODE;
		aw8623x_set_pwm(aw_haptic, AW_PWM_12K);
		aw8623x_auto_brk_config(aw_haptic, true);
		haptic_nv_i2c_write_bits(aw_haptic, AW8623X_REG_PLAYCFG3,
					 AW8623X_BIT_PLAYCFG3_PLAY_MODE_MASK,
					 AW8623X_BIT_PLAYCFG3_PLAY_MODE_RAM);
		aw8623x_vbat_mode_config(aw_haptic, AW_CONT_VBAT_HW_COMP_MODE);
		break;
	case AW_CONT_MODE:
...
static void aw8623x_irq_clear(struct aw_haptic *aw_haptic)
{
	...
	haptic_nv_i2c_reads(aw_haptic, AW8623X_REG_SYSINT, &val, AW_I2C_BYTE_ONE);
...
static void aw8623x_get_vbat(struct aw_haptic *aw_haptic)
{
	...
	aw8623x_play_stop(aw_haptic);
	aw8623x_raminit(aw_haptic, true);
	haptic_nv_i2c_write_bits(aw_haptic, AW8623X_REG_DETCFG2, AW8623X_BIT_DETCFG2_DET_SEQ0_MASK,
				 AW8623X_BIT_DETCFG2_DET_SEQ0_VBAT);
	haptic_nv_i2c_write_bits(aw_haptic, AW8623X_REG_DETCFG1, AW8623X_BIT_DETCFG1_DET_GO_MASK,
				 AW8623X_BIT_DETCFG1_DET_GO_ON);
	usleep_range(3000, 3500);
	haptic_nv_i2c_write_bits(aw_haptic, AW8623X_REG_DETCFG1, AW8623X_BIT_DETCFG1_DET_GO_MASK,
				 AW8623X_BIT_DETCFG1_DET_GO_OFF);
	haptic_nv_i2c_reads(aw_haptic, AW8623X_REG_DETRD1, reg_val, AW_I2C_BYTE_TWO);
...
{
	haptic_nv_i2c_writes(aw_haptic, AW8623X_REG_PLAYCFG2, &gain, AW_I2C_BYTE_ONE);
}
...
static void aw8623x_cont_config(struct aw_haptic *aw_haptic)
{
	...
	haptic_nv_i2c_writes(aw_haptic, AW8623X_REG_CONTCFG7, &reg_val, AW_I2C_BYTE_ONE);
	/* DRV2_TIME */
	reg_val = 0xFF;
	haptic_nv_i2c_writes(aw_haptic, AW8623X_REG_CONTCFG9, &reg_val, AW_I2C_BYTE_ONE);
	/* cont play go */
	aw8623x_play_go(aw_haptic);
}

static ssize_t aw8623x_get_reg(struct aw_haptic *aw_haptic, ssize_t len, char *buf)
{
	len = haptic_nv_read_reg_array(aw_haptic, buf, len, AW8623X_REG_RSTCFG,
				       AW8623X_REG_RTPDATA - 1);
	if (!len)
		return len;
	len = haptic_nv_read_reg_array(aw_haptic, buf, len, AW8623X_REG_RTPDATA + 1,
				       AW8623X_REG_RAMDATA - 1);
	if (!len)
		return len;
	len = haptic_nv_read_reg_array(aw_haptic, buf, len, AW8623X_REG_RAMDATA + 1,
				       AW8623X_REG_ANACFG10);
	return len;
}

static void aw8623x_protect_config(struct aw_haptic *aw_haptic, uint8_t prtime, uint8_t prlvl)
{
	uint8_t reg_val = 0;

	haptic_nv_i2c_write_bits(aw_haptic, AW8623X_REG_PWMCFG1, AW8623X_BIT_PWMCFG1_PRC_EN_MASK,
	...
	if (prlvl != 0) {
		...
		haptic_nv_i2c_writes(aw_haptic, AW8623X_REG_PWMCFG3, &reg_val, AW_I2C_BYTE_ONE);
		haptic_nv_i2c_writes(aw_haptic, AW8623X_REG_PWMCFG4, &prtime, AW_I2C_BYTE_ONE);
	} else {
		/* Disable */
...
static void aw8623x_misc_para_init(struct aw_haptic *aw_haptic)
{
	...
	aw_info("enter");
	/* Get seq and gain */
	haptic_nv_i2c_reads(aw_haptic, AW8623X_REG_WAVCFG1, &val, AW_I2C_BYTE_ONE);
	aw_haptic->index = val & AW8623X_BIT_WAVCFG_SEQ;
	haptic_nv_i2c_reads(aw_haptic, AW8623X_REG_PLAYCFG2, &val, AW_I2C_BYTE_ONE);
	aw_haptic->gain = val;
	aw_info("gain=0x%02X, index=0x%02X", aw_haptic->gain, aw_haptic->index);
	haptic_nv_i2c_reads(aw_haptic, AW8623X_REG_WAVCFG1, array, AW_SEQUENCER_SIZE);
	memcpy(aw_haptic->seq, array, AW_SEQUENCER_SIZE);
	/* GAIN_BYPASS config */
	haptic_nv_i2c_write_bits(aw_haptic, AW8623X_REG_SYSCTRL1,
				 AW8623X_BIT_SYSCTRL1_GAIN_BYPASS_MASK,
				 aw_haptic->info.gain_bypass << 3);
	...
	aw8623x_reg_unlock(aw_haptic, true);
	/* uvlo adj default 2.4V */
	haptic_nv_i2c_write_bits(aw_haptic, AW8623X_REG_ANACFG5,
				AW8623X_BIT_ANACFG5_UVLO_ADJ_MASK,
				AW8623X_BIT_ANACFG5_UVLO_ADJ_2_4_V);
	aw8623x_reg_unlock(aw_haptic, false);

	if (aw_haptic->info.cont_brk_time) {
		val = (uint8_t)aw_haptic->info.cont_brk_time;
		haptic_nv_i2c_writes(aw_haptic, AW8623X_REG_CONTCFG10, &val, AW_I2C_BYTE_ONE);
	}
	...
	aw8623x_vbat_ref_config(aw_haptic, AW_VBAT_REFER);
...
static int aw8623x_offset_cali(struct aw_haptic *aw_haptic)
{
	uint8_t reg_val[2] = {0};
	int os_code = 0;
	int d2s_gain = 0;
	...
	d2s_gain = aw8623x_select_d2s_gain(aw_haptic->info.d2s_gain);
	if (d2s_gain <= 0) {
...
static void aw8623x_calculate_cali_data(struct aw_haptic *aw_haptic)
{
	...
	f0_cali_step = 100000 * ((int)aw_haptic->f0 - (int)aw_haptic->info.f0_pre) /
				((int)aw_haptic->info.f0_pre * AW8623X_F0_CALI_ACCURACY);
...
static void aw8623x_read_cont_f0(struct aw_haptic *aw_haptic)
{
	...
	aw_info("enter");
	haptic_nv_i2c_reads(aw_haptic, AW8623X_REG_CONTRD16, val, AW_I2C_BYTE_TWO);
...
static int aw8623x_read_lra_f0(struct aw_haptic *aw_haptic)
{
	...
	aw_info("enter");
	/* F_LRA_F0 */
	haptic_nv_i2c_reads(aw_haptic, AW8623X_REG_CONTRD14, val, AW_I2C_BYTE_TWO);
	f0_reg = (f0_reg | val[0]) << 8;
	f0_reg |= (val[1] << 0);
	if (!f0_reg) {
		aw_haptic->f0 = 0;
...
static int aw8623x_get_f0(struct aw_haptic *aw_haptic)
{
	...
	aw8623x_vbat_ref_config(aw_haptic, AW_CONT_F0_VBAT_REFER);
	...
	haptic_nv_i2c_write_bits(aw_haptic, AW8623X_REG_CONTCFG6,
				 ...
				 (aw_haptic->info.is_enabled_track_en << 7));
	/* enable auto brake */
	haptic_nv_i2c_reads(aw_haptic, AW8623X_REG_PLAYCFG3, &val[0], AW_I2C_BYTE_ONE);
	brk_en_temp = AW8623X_BIT_PLAYCFG3_BRK & val[0];
	haptic_nv_i2c_write_bits(aw_haptic, AW8623X_REG_PLAYCFG3, AW8623X_BIT_PLAYCFG3_BRK_EN_MASK,
				 AW8623X_BIT_PLAYCFG3_BRK_ENABLE);
	/* f0 driver level */
	haptic_nv_i2c_write_bits(aw_haptic, AW8623X_REG_CONTCFG6,
				 AW8623X_BIT_CONTCFG6_DRV1_LVL_MASK, aw_haptic->info.cont_drv1_lvl);
	...
	haptic_nv_i2c_writes(aw_haptic, AW8623X_REG_CONTCFG8, val, AW_I2C_BYTE_TWO);
	/* TRACK_MARGIN */
	if (!aw_haptic->info.cont_track_margin) {
		aw_err("dts_info.cont_track_margin = 0!");
	} else {
		val[0] = (uint8_t)aw_haptic->info.cont_track_margin;
		haptic_nv_i2c_writes(aw_haptic, AW8623X_REG_CONTCFG11, &val[0], AW_I2C_BYTE_ONE);
	}
	/* DRV_WIDTH */
	if (!aw_haptic->info.f0_pre)
		return -ERANGE;
	drv_width = AW8623X_DRV_WIDTH_FORMULA(aw_haptic->info.f0_pre,
	...
	haptic_nv_i2c_writes(aw_haptic, AW8623X_REG_CONTCFG3, &val[0], AW_I2C_BYTE_ONE);
	/* cont play go */
	aw8623x_play_go(aw_haptic);
	usleep_range(20000, 20500);
	while (cnt) {
		haptic_nv_i2c_reads(aw_haptic, AW8623X_REG_GLBRD5, &val[0], AW_I2C_BYTE_ONE);
...
static void aw8623x_set_base_addr(struct aw_haptic *aw_haptic)
{
	...
	haptic_nv_i2c_write_bits(aw_haptic, AW8623X_REG_RTPCFG1, AW8623X_BIT_RTPCFG1_ADDRH_MASK,
				 (uint8_t)AW_SET_BASEADDR_H(base_addr));
	val = (uint8_t)AW_SET_BASEADDR_L(base_addr);
	haptic_nv_i2c_writes(aw_haptic, AW8623X_REG_RTPCFG2, &val, AW_I2C_BYTE_ONE);
}
...
static void aw8623x_set_fifo_addr(struct aw_haptic *aw_haptic)
{
	...
	haptic_nv_i2c_writes(aw_haptic, AW8623X_REG_RTPCFG3, val, AW_I2C_BYTE_THREE);
}
...
static void aw8623x_get_fifo_addr(struct aw_haptic *aw_haptic)
{
	...
	haptic_nv_i2c_reads(aw_haptic, AW8623X_REG_RTPCFG3, val, AW_I2C_BYTE_THREE);
...
static void aw8623x_set_ram_addr(struct aw_haptic *aw_haptic)
{
	...
	haptic_nv_i2c_writes(aw_haptic, AW8623X_REG_RAMADDRH, val, AW_I2C_BYTE_TWO);
}

static void aw8623x_set_ram_data(struct aw_haptic *aw_haptic, uint8_t *data, int len)
{
	haptic_nv_i2c_writes(aw_haptic, AW8623X_REG_RAMDATA, data, len);
}

static void aw8623x_get_first_wave_addr(struct aw_haptic *aw_haptic, uint32_t *first_wave_addr)
{
	uint8_t val[3] = {0};

	haptic_nv_i2c_reads(aw_haptic, AW8623X_REG_RAMDATA, val, AW_I2C_BYTE_THREE);
...
static void aw8623x_parse_dts(struct aw_haptic *aw_haptic, struct device_node *np)
{
	uint32_t duration_time[3];
	uint32_t trig_config_temp[21];
	uint32_t val = 0;

	val = of_property_read_u32(np, "aw8623x_gain_bypass", &aw_haptic->info.gain_bypass);
	if (val != 0)
		aw_info("aw8623x_gain_bypass not found");
	val = of_property_read_u32(np, "aw8623x_vib_lk_f0_cali", &aw_haptic->info.lk_f0_cali);
	if (val != 0)
		aw_info("aw8623x_vib_lk_f0_cali not found");
	val = of_property_read_u32(np, "aw8623x_vib_mode", &aw_haptic->info.mode);
	if (val != 0)
		aw_info("aw8623x_vib_mode not found");
	val = of_property_read_u32(np, "aw8623x_vib_f0_pre", &aw_haptic->info.f0_pre);
	if (val != 0)
		aw_info("vib_f0_pre not found");
	val = of_property_read_u32(np, "aw8623x_vib_f0_cali_percen",
				   &aw_haptic->info.f0_cali_percent);
	if (val != 0)
		aw_info("vib_f0_cali_percent not found");
	val = of_property_read_u32(np, "aw8623x_vib_cont_drv1_lvl", &aw_haptic->info.cont_drv1_lvl);
	if (val != 0)
		aw_info("vib_cont_drv1_lvl not found");
	val = of_property_read_u32(np, "aw8623x_vib_lra_vrms", &aw_haptic->info.lra_vrms);
	if (val != 0)
		aw_info("vib_cont_lra_vrms not found");
	val = of_property_read_u32(np, "aw8623x_vib_cont_brk_time", &aw_haptic->info.cont_brk_time);
	if (val != 0)
		aw_info("vib_cont_brk_time not found");
	val = of_property_read_u32(np, "aw8623x_vib_cont_brk_gain", &aw_haptic->info.cont_brk_gain);
	if (val != 0)
		aw_info("vib_cont_brk_gain not found");
	val = of_property_read_u32(np, "aw8623x_vib_cont_drv1_time",
				   &aw_haptic->info.cont_drv1_time);
	if (val != 0)
		aw_info("vib_cont_drv1_time not found");
	val = of_property_read_u32(np, "aw8623x_vib_cont_drv2_time",
				   &aw_haptic->info.cont_drv2_time);
	if (val != 0)
		aw_info("vib_cont_drv2_time not found");
	val = of_property_read_u32(np, "aw8623x_vib_cont_track_margin",
				   &aw_haptic->info.cont_track_margin);
	if (val != 0)
		aw_info("vib_cont_track_margin not found");
	val = of_property_read_u32(np, "aw8623x_vib_d2s_gain", &aw_haptic->info.d2s_gain);
	if (val != 0)
		aw_info("vib_d2s_gain not found");
	val = of_property_read_u32_array(np, "aw8623x_vib_trig_config", trig_config_temp,
	...
	val = of_property_read_u32_array(np, "aw8623x_vib_duration_time", duration_time,
					 ARRAY_SIZE(duration_time));
	if (val != 0)
		aw_info("aw8623x_vib_duration_time not found");
	else
		memcpy(aw_haptic->info.duration_time, duration_time, sizeof(duration_time));
	aw_haptic->info.is_enabled_track_en =
		of_property_read_bool(np, "aw8623x_vib_is_enabled_track_en");
...
static void aw8623x_set_wav_seq(struct aw_haptic *aw_haptic, uint8_t wav, uint8_t seq)
{
	haptic_nv_i2c_writes(aw_haptic, AW8623X_REG_WAVCFG1 + wav, &seq, AW_I2C_BYTE_ONE);
}

static void aw8623x_set_wav_loop(struct aw_haptic *aw_haptic, uint8_t wav, uint8_t loop)
{
	uint8_t tmp = 0;

	if (wav % 2) {
		tmp = loop << 0;
		haptic_nv_i2c_write_bits(aw_haptic, AW8623X_REG_WAVCFG9 + (wav / 2),
...
{
	haptic_nv_i2c_reads(aw_haptic, AW8623X_REG_PLAYCFG2, gain, AW_I2C_BYTE_ONE);
}

static void aw8623x_get_wav_loop(struct aw_haptic *aw_haptic, uint8_t *val)
{
	haptic_nv_i2c_reads(aw_haptic, AW8623X_REG_WAVCFG9, val, AW_SEQUENCER_LOOP_SIZE);
}

static void aw8623x_get_ram_data(struct aw_haptic *aw_haptic, uint8_t *ram_data, int size)
{
...
static void aw8623x_get_lra_resistance(struct aw_haptic *aw_haptic)
{
	...
	haptic_nv_i2c_reads(aw_haptic, AW8623X_REG_DETCFG1, &adc_fs, AW_I2C_BYTE_ONE);
	haptic_nv_i2c_reads(aw_haptic, AW8623X_REG_DETCFG2, reg_val, AW_I2C_BYTE_ONE);
	...
	haptic_nv_i2c_write_bits(aw_haptic, AW8623X_REG_DETCFG1, AW8623X_BIT_DETCFG1_ADC_FS_MASK,
	...
	haptic_nv_i2c_write_bits(aw_haptic, AW8623X_REG_DETCFG1, AW8623X_BIT_DETCFG1_ADC_FS_MASK,
				 adc_fs);
	/* restore d2s_gain */
	haptic_nv_i2c_write_bits(aw_haptic, AW8623X_REG_DETCFG2, AW8623X_BIT_DETCFG2_D2S_GAIN_MASK,
				 d2s_gain_default);
	haptic_nv_i2c_reads(aw_haptic, AW8623X_REG_DETRD1, reg_val, AW_I2C_BYTE_TWO);
...
static uint8_t aw8623x_get_prctmode(struct aw_haptic *aw_haptic)
{
	...
	haptic_nv_i2c_reads(aw_haptic, AW8623X_REG_PWMCFG3, &reg_val, AW_I2C_BYTE_ONE);
	reg_val >>= 7;
...
static uint64_t aw8623x_get_theory_time(struct aw_haptic *aw_haptic)
{
	...
	haptic_nv_i2c_reads(aw_haptic, AW8623X_REG_SYSCTRL2, &reg_val, AW_I2C_BYTE_ONE);
...
static uint8_t aw8623x_get_osc_status(struct aw_haptic *aw_haptic)
{
	...
	haptic_nv_i2c_reads(aw_haptic, AW8623X_REG_SYSST2, &state, AW_I2C_BYTE_ONE);
...
static int aw8623x_check_qualify(struct aw_haptic *aw_haptic)
{
	...
	aw_info("enter");
	ret = haptic_nv_i2c_reads(aw_haptic, AW8623X_REG_EFCFG6, &reg, AW_I2C_BYTE_ONE);
	if (ret < 0)
		return ret;
	if ((reg & 0x80) == 0x80)
		return 0;
				   ...
				   const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		...
		haptic_nv_i2c_writes(aw_haptic, AW8623X_REG_CONTCFG8, reg_val, AW_I2C_BYTE_TWO);
	}
	mutex_unlock(&aw_haptic->lock);
				   ...
				   const char *buf, size_t count)
{
	...
	haptic_nv_i2c_writes(aw_haptic, AW8623X_REG_CONTCFG10, &reg_val, AW_I2C_BYTE_ONE);
	mutex_unlock(&aw_haptic->lock);
...
static ssize_t rtp_autosin_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	len = snprintf(buf, PAGE_SIZE, "rtp_cnt = %u\n", aw_haptic->rtp_cnt);

	return len;
}

static ssize_t rtp_autosin_store(struct device *dev, struct device_attribute *attr, const char *buf,
			  size_t count)
{
	uint32_t val = 0;
	int rc = 0;
	...
	if (!aw_haptic->ram.base_addr) {
		aw_err("no set FIFO space for autosin rtp play!");
		return count;
	}
	...
	if ((val > 0) && (val < aw_haptic->rtp_num_max)) {
		...
		haptic_nv_i2c_write_bits(aw_haptic, AW8623X_REG_RTPCFG6,
...
static DEVICE_ATTR_RW(rtp_autosin);
...
static void aw8623x_creat_node(struct aw_haptic *aw_haptic)
{
	...
	ret = sysfs_create_group(&aw_haptic->vib_dev.dev->kobj, &aw8623x_vibrator_attribute_group);
	if (ret < 0)
...
struct aw_haptic_func aw8623x_func_list = {
	...
	.rtp_get_fifo_aes = aw8623x_rtp_get_fifo_aes,
